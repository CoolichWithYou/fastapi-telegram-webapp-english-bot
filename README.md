...
|          general | запустить основные контейнеры                    |
...
Также дополнительные переменные среды должны находится в ./front/src для запуска фронта локально:

|     Переменная среды | Пример значения по умолчанию | Краткое описание                                      |
|---------------------:|:-----------------------------|-------------------------------------------------------|
|         VITE_API_URL | http://localhost:8000        | адрес бека                                            |
| VITE_DEFAULT_CHAT_ID | 416248077                    | id пользователя телеграм. Можно узнать у @userinfobot |
|                DEBUG | True                         | запуск в дебаг режиме или нет                         |
